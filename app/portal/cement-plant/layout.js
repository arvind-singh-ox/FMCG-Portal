...
import Sidebar from '@/components/industries/cement-plant/components/Sidebar'
import TopBar from '@/components/industries/cement-plant/components/TopBar'
import Chatbot from '@/components/industries/cement-plant/components/Chatbot'

const ACCENT = '#605dba'

export default function CementPlantLayout({ children }) {
  const params = useParams()
  const router = useRouter()
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)

  const activeItem = params?.section || 'overview'
  const setActiveItem = (key) => router.push(`/portal/cement-plant/${key}`)
      ...
      {showDemoPopup && (
        <div style={styles.popupOverlay}>
          <div style={styles.popup}>
            ...
            <h3 style={styles.popupTitle}>See How iFactory AI Can Transform Your Plant</h3>
            <p style={styles.popupText}>
              This is a preview of our platform experience. To unlock a fully customized, production-ready solution tailored to your plant operations — schedule a quick 30-min call with our team.
            </p>
            <div style={styles.popupBenefits}>
              {['Live walkthrough of your plant use-case', 'Custom ROI analysis for your operations', 'On-premise deployment discussion'].map((b, i) => (
